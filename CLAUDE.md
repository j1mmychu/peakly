...
9. **Venue data is hardcoded** — `VENUES` array has **404 entries** (2 launch categories: skiing and beach — 134 skiing, 270 beach; surfing retired 2026-05-03). Weather fetching is batched (50/2s) to avoid Open-Meteo rate limits. Cached in localStorage with 2hr TTL. Marine API only fetched for beach venues (water temp only).
...
4. ~~**`lateSeason: true` flag never wired up on any ski venue**~~ **DONE 2026-05-04 (expanded through 2026-05-05)** — **Current count: 15** (confirmed via grep 2026-09-13): whistler, chamonix, mammoth, abasin, tignes, hintertux-glacier, cervinia, snowbird, zermatt, verbier, val-thorens, les-deux-alpes-fr, saas-fee-ch, st-moritz-ch, engelberg. Previous CLAUDE.md counts are stale — always `grep -c "lateSeason:true"` in app.jsx for the authoritative count.
