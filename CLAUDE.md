...
- ⚠️ **"+134 beach venues (156 → 290)" (commit `7561a18`) DID NOT LAND** — the commit message claims the batch shipped, but a direct count of the VENUES array at that commit's tree shows beach still at 89 (total 156). The 2564 lines the commit added were the tag-chip removal + reformat, not venues. **Real count is and has stayed 156.** The validated batch in `data/` was never actually pasted into `app.jsx`. If the extra venues are still wanted, re-do the paste through `validate-venues.mjs` and verify the count climbs. Corrected 2026-06-09.
