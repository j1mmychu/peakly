...
- ✅ **+134 beach venues shipped (156 → 290)** (commit `7561a18`) — the "Global beach batch" that had been sitting uncommitted in the working tree was validated clean via `scripts/validate-venues.mjs` (0 dup IDs within batch + vs existing, coords/required-fields/airport-mapping all pass; only soft flag was ~80 out-of-vocab descriptive tags, cosmetic). Shipped as a labeled commit (paper trail per Open #14). NOTE: weather batching now spans 290 venues — leans harder on the (now-confirmed-live) VPS weather cache at scale.
