...
6. ~~**Travelpayouts weekend-specific dates not wired**~~ **DONE 2026-05-05** — `fetchTravelpayoutsPrice(origin, dest, departDate, returnDate)` now passes Fri–Mon dates derived from `weekendISODates(today)` (app.jsx:1493). Server `/api/flights` accepts optional `depart_date` + `return_date` query params and routes to Travelpayouts `v1/prices/cheap` for exact-weekend pricing; falls back to month-matrix when dates absent. Flight cache key now includes the dates so last weekend's price never lingers. Needs VPS redeploy.
7. ~~**Open-Meteo weather cache still unbuilt**~~ **DONE 2026-05-05** — VPS proxy now serves `/api/weather/forecast` and `/api/marine/marine` with a 30-min server-side Map cache (lat/lon rounded to 0.1° for shared hits). Client routes Open-Meteo through the proxy first and falls back to direct on 404/network error (`_fetchWeatherJson` helper, app.jsx:838) so the deploy/merge ordering is decoupled. Reddit-spike protection: 100 visitors loading Whistler → 1 upstream call. Needs VPS redeploy.
8. ~~**No onboarding scoring explanation**~~ **DONE 2026-05-05** — added step 3 "How the score works" to `OnboardingSheet` (app.jsx:6135) between Sports and Complete. Sample green 92 score chip + three explainers (best 2 of 4 days, live conditions, confidence flag). Progress dots bumped 2 → 3.
9. ~~**Strike alerts server polling**~~ **DONE 2026-05-05** — `server/alerts-worker.js` polls every 30 min using a verbatim port of `scoreVenue`/`scoreWeekend` (`server/scoring.js`); web push delivers via VAPID; alerts persist to `server/data/alerts.json`. Client `addAlert` POSTs the full venue snapshot + PushSubscription to `/api/alerts`; SW push handler was already wired in `sw.js`. Operational TODO: VPS needs `npm install` (adds `web-push`) + VAPID env vars (`VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` — generate with `npx web-push generate-vapid-keys`) + service restart. iOS APNs intentionally still stubbed (logs intent only — needs Apple .p8 cert wiring).
