...
- ✅ Push notification icon format fixed
...
`app.jsx:7214`: Replace `alert("You're on the list!")` with a fetch POST to `https://peakly-api.duckdns.org/api/waitlist` with `{email}`, then show inline success copy in the form. The `/api/waitlist` server endpoint was added in the April 9 merge. The client side just needs to call it.
...
Every AI session reading the file will think these are closed. They're not.
...
1. **TP_MARKER set this week.** You have zero flight conversion data. You need it to know if the core feature is working.
